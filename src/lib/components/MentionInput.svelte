<script lang="ts">
	...
	const filtered = $derived(() => {
		if (!mentionQuery) return [{ id: 'everyone', display: 'everyone', sub: 'Notify all RSVPs' }, ...members.map(m => ({ id: m.id, display: m.bash_name || m.christian_name, sub: m.bash_name ? m.christian_name : null }))]
		...
		for (const m of members) {
			...
			if (display.toLowerCase().includes(q) || m.christian_name.toLowerCase().includes(q) || (m.bash_name && m.bash_name.toLowerCase().includes(q))) {
...
<div class="relative">
	...
	{#if showDropdown && filtered().length > 0}
		<div class="absolute bottom-full left-0 z-20 mb-1 w-full max-h-48 overflow-y-auto rounded-lg border border-gray-700 bg-gray-800 shadow-lg">
			{#each filtered() as item, i}
				<button
					type="button"
					onmousedown={(e) => { e.preventDefault(); insertMention(item.display) }}
					class="flex w-full items-center gap-2 px-3 py-2 text-left text-sm transition {i === selectedIndex ? 'bg-blue-600/30 text-gray-100' : 'text-gray-300 hover:bg-gray-700'}"
				>
